...
#else
    const BIGNUM *ecs_r, *ecs_s;
    ECDSA_SIG_get0(ecsig, &ecs_r, &ecs_s);
    if (!BN_add(x, x, ecs_r)) { ret = -1; goto err; }
#endif
...
#else
    if (!BN_mod_inverse(rr, ecs_r, order, ctx)) { ret = -1; goto err; }
#endif
    sor = BN_CTX_get(ctx);
#if OPENSSL_VERSION_NUMBER < 0x10100000
    if (!BN_mod_mul(sor, ecsig->s, rr, order, ctx)) { ret=-1; goto err; }
#else
    if (!BN_mod_mul(sor, ecs_s, rr, order, ctx)) { ret = -1; goto err; }
#endif
	eor = BN_CTX_get(ctx);
...
bool CKey::Sign(uint256 hash, std::vector<unsigned char>& vchSig)
{
    ...
    if (BN_cmp(sig_s, halforder) > 0) {
        // enforce low S values, by negating the value (modulo the order) if above order/2.
	BN_sub((BIGNUM *)sig_s, order, sig_s);
    }
#endif
    BN_CTX_end(ctx);
    BN_CTX_free(ctx);
...
bool CKey::SetCompactSignature(uint256 hash, const std::vector<unsigned char>& vchSig)
{
    ...
    const BIGNUM *sig_r, *sig_s;
    ECDSA_SIG_get0(sig, &sig_r, &sig_s);
    BN_bin2bn(&vchSig[1], 32, (BIGNUM *)sig_r);
    BN_bin2bn(&vchSig[33], 32, (BIGNUM *)sig_s);
#endif
    EC_KEY_free(pkey);
    pkey = EC_KEY_new_by_curve_name(NID_secp256k1);
    if (nV >= 31)
